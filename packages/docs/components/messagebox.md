...
调用 `CtMessageBox.alert` 方法以打开 alert 框。
...
调用 `CtMessageBox.confirm` 方法以打开 confirm 框。
...
调用 `CtMessageBox.prompt` 方法以打开 prompt 框。
...
可以通过配置 `CtMessageBox` 的参数来实现一些个性化需求（options 参数见下文）。
...
`CtMessageBox` 提供了全局方法 `CtMessageBox.alert`、`CtMessageBox.confirm`、`CtMessageBox.prompt`，用于在需要时弹出消息弹框。
如果完整引入了 `CticUI`, 则会为 `app.config.globalProperties` 添加全局方法 `$msgbox`、`$alert`、`$confirm`、`$prompt`。
在 Vue 实例中可以作为 `this.$msgbox`、`this.$alert`、`this.$confirm`、`this.$prompt` 使用。

## 单独引用

```typescript
import { CtMessageBox } from 'create-element'
```
