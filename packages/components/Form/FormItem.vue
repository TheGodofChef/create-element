<template>
  <div class="ct-form-item" :class="{
    ...
    <component v-if="hasLabel" class="ct-form-item__label" :class="`position-${ctx?.labelPosition ?? `right`}`"
    ...
    <div class="ct-form-item__content">
      ...
      <div class="ct-form-item__error-msg" v-if="validateStatus === 'error'">
...
const addInputId: FormItemContext['addInputId'] = function (id: string) {
  if (!includes(inputIds.value, id)) inputIds.value.push(id)
}

const removeInputId: FormItemContext['removeInputId'] = function (id: string) {
...
.ct-form-item {
  --ct-form-label-width: v-bind(normalizeLabelWidth) !important;
}
</style>